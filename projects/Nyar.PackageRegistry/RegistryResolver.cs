...
public class RegistryResolver : IRegistryResolver
{
    ...
    {
        if (_registries.TryGetValue(registryName, out var registry)) return registry;
    ...
    /// <inheritdoc />
    public T? get_registry<T>() where T : class, IRegistry
    {
    ...
    /// <inheritdoc />
    public IReadOnlyDictionary<string, IRegistry> get_all_registries()
    {
        return _registries;
    }

    /// <inheritdoc />
    public ISet<string> get_registry_names()
    {
