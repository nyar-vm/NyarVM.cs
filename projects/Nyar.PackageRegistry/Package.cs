...
public class Package
{
    ...
    ///     值中列出该目标下有效的额外依赖
    /// </summary>
    public Dictionary<string, List<string>> target_conditions { get; set; } = new();

    /// <summary>
    ///     分发 Tarball/Jar 等二进制文件的下载地址
    /// </summary>
    public string? dist_tarball { get; set; }

    /// <summary>
    ///     分发文件完整性校验值（SRI 格式）
    /// </summary>
    public string? dist_integrity { get; set; }

    /// <summary>
    ///     是否为 SDK 包（如 std.adaptor.wasm、std.adaptor.dotnet）
    /// </summary>
    public bool is_sdk_package { get; set; }
