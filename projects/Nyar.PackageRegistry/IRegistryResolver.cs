...
public interface IRegistryResolver
{
    ...
    IRegistry? get_registry(string registryName);
    ...
    /// <returns>注册表实例，未找到返回 null</returns>
    T? get_registry<T>() where T : class, IRegistry;

    /// <summary>
    ///     获取所有已注册的注册表
    /// </summary>
    /// <returns>注册表名称到实例的映射</returns>
    IReadOnlyDictionary<string, IRegistry> get_all_registries();

    /// <summary>
    ///     获取所有已注册的注册表名称
    /// </summary>
    ISet<string> get_registry_names();
}