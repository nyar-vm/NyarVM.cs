...
/// <summary>
/// 火山引擎 TOS 对象存储实现，使用 TOS4-HMAC-SHA256 签名（类似 AWS Signature V4）
/// </summary>
public sealed class VolcengineTosStorage : IBlobStorage
{
    ...
    private const string _service = "tos";
    ...
    /// <param name="endpoint">TOS 端点，如 tos-cn-beijing.volces.com</param>
    /// <param name="httpClient">HTTP 客户端</param>
    public VolcengineTosStorage(
        ...
        CancellationToken cancel = default)
    {
        ...
        return BlobResult.fail($"TOS 上传失败: {response.StatusCode} - {errorBody}");
    }
    ...
    public async Task<BlobResult> delete_object(string bucket, string key, CancellationToken cancel = default)
    {
        ...
        var errorBody = await response.Content.ReadAsStringAsync(cancel);
        return BlobResult.fail($"TOS 删除失败: {response.StatusCode} - {errorBody}");
    }
    ...
    public string generate_presigned_url(string bucket, string key, TimeSpan expiry, string method = "GET")
    {
        ...
        var tosDate = t.ToString("yyyyMMddTHHmmssZ");
        var datestamp = t.ToString("yyyyMMdd");
        var credential = $"{_access_key}/{datestamp}/{_region}/{_service}/request";

        var canonicalRequest = $"{method}\n/{bucket}/{key}\n\nhost:{_endpoint}\n\nhost\nUNSIGNED-PAYLOAD";
        var stringToSign = $"TOS4-HMAC-SHA256\n{tosDate}\n{datestamp}/{_region}/{_service}/request\n{sha256_hex(canonicalRequest)}";
        ...
        return $"https://{_endpoint}/{bucket}/{Uri.EscapeDataString(key)}"
               ...
               + $"&X-Tos-Credential={Uri.EscapeDataString(credential)}"
               + $"&X-Tos-Date={tosDate}"
               + $"&X-Tos-Expires={(int)expiry.TotalSeconds}"
        ...
        string? queryString = null)
    {
        ...
        request.Headers.Add("X-Tos-Date", tosDate);

        var payloadHash = body is not null ? sha256_hex(body) : sha256_hex(string.Empty);
        request.Headers.Add("X-Tos-Content-Sha256", payloadHash);

        var signedHeaders = "host;x-tos-content-sha256;x-tos-date";
        ...
        request.Headers.Add("Authorization",
            $"TOS4-HMAC-SHA256 Credential={_access_key}/{credentialScope}, SignedHeaders={signedHeaders}, Signature={signature}");
    }
    ...
    private byte[] derive_signing_key(string datestamp)
    {
        return hmac_sha256(
            hmac_sha256(
                hmac_sha256(
                    hmac_sha256(
                        Encoding.UTF8.GetBytes($"TOS4{_secret_key}"),
                        datestamp),
                    _region),
                _service),
            "request");
