...
public class PackagePacker
{
    ...
    public static PackResult pack(string packageDirectory, LegionIgnore? ignore = null)
    {
        ...
        {
            var files = get_files_to_pack(packageDirectory, ignore);
            ...
            {
                packed_at = DateTime.UtcNow.ToString("O"),
                packer_version = "1.0.0"
            };

            if (!string.IsNullOrEmpty(manifestContent)) meta.has_manifest = true;
        ...
        {
            tarball_data = tarballData,
            sha256 = $"sha256-{Convert.ToHexString(hash).ToLowerInvariant()}",
            size = tarballData.Length,
            file_count = get_files_to_pack(packageDirectory, ignore).Count
        };
    }
    ...
    public static bool verify_integrity(byte[] tarballData, string expectedSha256)
    {
        using var sha256 = SHA256.Create();
        var hash = sha256.ComputeHash(tarballData);
    ...
    ///     获取需要打包的文件列表
    /// </summary>
    private static List<string> get_files_to_pack(string directory, LegionIgnore? ignore)
    {
        ...
        enumerate_files(directory, directory, alwaysExclude, alwaysExcludeFiles, ignore, result);

        return result;
    }

    private static void enumerate_files(
        string rootDir,
        string currentDir,
        ...
        List<string> result)
    {
        try
        {
            foreach (var file in Directory.GetFiles(currentDir))
            {
                ...
                if (ignore?.is_ignored(file[(rootDir.Length + 1)..]) == true) continue;
            ...
            foreach (var dir in Directory.GetDirectories(currentDir))
            {
                ...
                if (ignore?.is_ignored(dir[(rootDir.Length + 1)..]) == true) continue;

                enumerate_files(rootDir, dir, excludedDirs, excludedFiles, ignore, result);
            }
        }
