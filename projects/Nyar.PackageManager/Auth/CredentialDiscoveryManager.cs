using Nyar.PackageManager.CredentialProviders;
...
public class CredentialDiscoveryManager
{
    ...
    ///     注册凭据提供器
    /// </summary>
    public void register_provider(ICredentialProvider provider)
    {
    ...
    ///     注册默认的内置凭据提供器
    /// </summary>
    public void register_builtin_providers()
    {
    ...
    public async Task<DiscoveredCredential?> discover(string vendorName)
    {
        var candidates = _providers
            .Where(p => p.vendor_name.Equals(vendorName, StringComparison.OrdinalIgnoreCase))
            .ToList();
        ...
        foreach (var provider in candidates)
        {
            ...
            try
            {
                ...
                {
                    credential.source = $"{provider.provider_name} → {credential.source}";
                    return credential;
                }
            }
            catch
    ...
    /// </summary>
    public List<ICredentialProvider> get_available_providers(string vendorName)
    {
        return
        [
            .. _providers
                .Where(p => p.vendor_name.Equals(vendorName, StringComparison.OrdinalIgnoreCase) && p.is_available())
        ];
    }
}