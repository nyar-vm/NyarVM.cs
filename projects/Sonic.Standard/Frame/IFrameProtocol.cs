...
///         长度前缀（Wasm Section）、类的长�?数据（GLB Chunk）的
///         魔数+头部+载荷（PSD）、变长标的数据（Protobuf）等�?///
///     </para>
///     <para>
///         协议实现仅操�?see cref="ReadOnlySpan{T}" />�?///     �?see cref="FrameScanner{TProtocol}" /> 负责管理缓冲区位置推进，
...
public interface IFrameProtocol
{
    ...
    ///     <param name="frame">
    ///         如果成功则输出帧数据�?param>
    ///         <returns>如果成功读取一个完整帧则返的true�?returns>
