...
/// <summary>
///     四维向量，表示具有 x、y、z 和 w 分量的四维空间向量
/// </summary>
/// <typeparam name="T">分量数值类型，必须为值类型且支持相等比较和类型转换</typeparam>
public readonly struct Vec4<T> :
    IAdditive<Vec4<T>>,
    INegation<Vec4<T>, Vec4<T>>,
    IApproximate<Vec4<T>>,
    IEquatable<Vec4<T>>,
    IClone<Vec4<T>>,
    IZero<Vec4<T>>,
    ...
    where T : struct, IEquatable<T>, IConvertible
{
    ...
    /// <summary>閫忔槑搴﹂€氶亾鍒悕锛岀瓑浠蜂簬 w 鍒嗛�?/summary>
    public T a => w;
    ...
    ///     获取或设置指定索引处的分量
    /// </summary>
    /// <param name="index">分量索引，0 为 x，1 为 y，2 为 z，3 为 w</param>
    ...
    public T this[int index]
    {
        ...
        set => throw new NotSupportedException("Vec4 是不可变结构体，不支持索引器设置");
    }
    ...
    ///     鍙︿竴涓洓缁村悜閲?/param>
    ///     <returns>鐐圭Н�?/returns>
    public double dot(Vec4<T> other)
    {
        return to_d(x) * to_d(other.x) + to_d(y) * to_d(other.y) + to_d(z) * to_d(other.z) + to_d(w) * to_d(other.w);
    }
    ...
    {
        var len = length();
        if (len == 0.0) return new Vec4<T>(from_d(0), from_d(0), from_d(0), from_d(0));
    ...
    /// <summary>
    ///     璁＄畻涓庡彟涓€涓洓缁村悜閲忎箣闂寸殑娆у嚑閲屽緱璺濈
    /// </summary>
    /// <param name="other">鐩爣鍚戦噺</param>
    /// <returns>涓ょ偣闂磋窛�?/returns>
    public double distance_to(Vec4<T> other)
    {
        var dx = to_d(x) - to_d(other.x);
        var dy = to_d(y) - to_d(other.y);
        var dz = to_d(z) - to_d(other.z);
        var dw = to_d(w) - to_d(other.w);
        return SonicMath.sqrt(dx * dx + dy * dy + dz * dz + dw * dw);
    }

    /// <summary>
    ///     璁＄畻涓庡彟涓€涓洓缁村悜閲忎箣闂寸殑澶硅锛堝姬搴︼�?    ///
    /// </summary>
    /// <param name="other">
    ///     鍙︿竴涓洓缁村悜閲?/param>
    ///     <returns>澶硅寮у害鍊?/returns>
    public double angle_between(Vec4<T> other)
    {
    ...
    public static Vec4<T> scale(Vec4<T> v, T scalar)
    {
        var s = to_d(scalar);
        return new Vec4<T>(
            from_d(to_d(v.x) * s),
    ...
    /// <returns>缂╂斁鍚庣殑鍚戦�?/returns>
    public static Vec4<T> scale_inverse(Vec4<T> v, T scalar)
    {
        var s = to_d(scalar);
        return new Vec4<T>(
            from_d(to_d(v.x) / s),
    ...
    /// <param name="right">
    ///     鍙充晶鎿嶄綔�?/param>
    ///     <returns>鐩稿姞缁撴灉</returns>
    public Vec4<T> add(Vec4<T> right)
    {
        return new Vec4<T>(
            from_d(to_d(x) + to_d(right.x)),
            from_d(to_d(y) + to_d(right.y)),
            from_d(to_d(z) + to_d(right.z)),
            from_d(to_d(w) + to_d(right.w))
        );
    }
    ...
    /// <param name="right">
    ///     鍙充晶鎿嶄綔�?/param>
    ///     <returns>鐩稿噺缁撴灉</returns>
    public Vec4<T> sub(Vec4<T> right)
    {
        return new Vec4<T>(
            from_d(to_d(x) - to_d(right.x)),
            from_d(to_d(y) - to_d(right.y)),
    ...
    public Vec4<T> neg()
    {
        return new Vec4<T>(from_d(-to_d(x)), from_d(-to_d(y)), from_d(-to_d(z)), from_d(-to_d(w)));
    }
    ...
    /// <summary>
    ///     鍒ゆ柇涓や釜鍥涚淮鍚戦噺鏄惁鍦ㄧ粰瀹氳宸寖鍥村唴杩戜技鐩哥瓑锛岄€愬垎閲忔瘮杈?    ///
    /// </summary>
    ...
    public bool is_close(Vec4<T> other, double absoluteTolerance, double relativeTolerance)
    {
        return is_component_close(to_d(x), to_d(other.x), absoluteTolerance, relativeTolerance)
               ...
               && is_component_close(to_d(w), to_d(other.w), absoluteTolerance, relativeTolerance);
    }

    /// <summary>
    ///     判断两个四维向量是否在给定逐分量容差范围内近似相等
    /// </summary>
    /// <param name="other">比较目标</param>
    /// <param name="epsilon">逐分量容差</param>
    /// <returns>是否近似相等</returns>
    public bool approx_equals(Vec4<T> other, Vec4<T> epsilon)
    {
        return SonicMath.abs(to_d(x) - to_d(other.x)) <= to_d(epsilon.x)
               ...
               && SonicMath.abs(to_d(w) - to_d(other.w)) <= to_d(epsilon.w);
    }
    ...
    /// <summary>
    ///     鍒ゆ柇涓や釜鍥涚淮鍚戦噺鏄惁鐩哥瓑锛屾瘡涓垎閲忓潎鐩哥瓑鏃惰繑鍥?true
    /// </summary>
    /// <param name="other">姣旇緝鐩爣</param>
    ...
    /// <summary>
    ///     杩斿洖褰撳墠鍥涚淮鍚戦噺鐨勬繁鎷疯礉锛岀敱浜庢槸 readonly struct锛岀洿鎺ヨ繑鍥炶嚜韬嵆�?    ///
    /// </summary>
    ...
    ///     获取零向量 (0, 0, 0, 0)
    /// </summary>
    public static Vec4<T> zero => new(from_d(0), from_d(0), from_d(0), from_d(0));
    ...
    ///     获取单位向量 (1, 1, 1, 1)
    /// </summary>
    public static Vec4<T> one => new(from_d(1), from_d(1), from_d(1), from_d(1));
