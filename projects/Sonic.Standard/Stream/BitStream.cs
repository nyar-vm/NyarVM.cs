...
/// <remarks>
///     <para>
///         比特流适用于需要将数据压缩到比特级别的场景，例如网络同步、紧凑存储等�?///     写入时自动扩容，读取时按比特位解析�?///
///     </para>
///     <para>
///         内部采用 LSB-first 比特排列：比�?0 位于字节 0 的最低位�?///     字节对齐时使用直接写�?读取，非对齐时按字节块处理，
///         相比逐位循环可获得最�?8 倍的吞吐提升�?///
///     </para>
/// </remarks>
public ref struct BitStream
{
    ...
    public uint read_bits(int bits)
    {
        ...
        if (bit_position + bits > bit_length)
            throw new InvalidOperationException($"比特流剩�?{bit_length - bit_position} 比特，不足以读取 {bits} 比特");
