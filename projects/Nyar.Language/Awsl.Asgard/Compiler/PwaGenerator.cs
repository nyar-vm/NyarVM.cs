...
public static class PwaGenerator
{
    ...
    /// </summary>
    public static string generate_service_worker(string cacheName, string[] assets)
    {
        ...
        sb.AppendLine(
            "          return name.startsWith(CACHE_PREFIX) && name !== CACHE_NAME && name !== RUNTIME_CACHE_NAME;");
    ...
    /// </summary>
    public static string generate_manifest(string name, string shortName,
    ...
    /// </summary>
    public static string generate_registration_script()
    {
        var sb = new StringBuilder();
        ...
        sb.AppendLine("    banner.innerHTML = [");
        sb.AppendLine(
            "      '<div style=\"position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:#1a1a2e;color:#e0e0ff;padding:16px 24px;border-radius:12px;z-index:99998;box-shadow:0 4px 24px rgba(0,0,0,0.3);font-family:system-ui,sans-serif;font-size:14px;border:1px solid #7c3aed\">',");
        sb.AppendLine("      '<span>将此应用安装到桌面</span>',");
        sb.AppendLine(
            "      '<button id=\"voa-install-btn\" style=\"margin-left:12px;background:#7c3aed;color:#fff;border:none;padding:6px 16px;border-radius:6px;cursor:pointer;font-weight:600\">安装</button>',");
        sb.AppendLine(
            "      '<button onclick=\"this.parentElement.parentElement.remove()\" style=\"margin-left:8px;background:transparent;color:#888;border:none;padding:6px 8px;cursor:pointer\">✕</button>',");
        sb.AppendLine("      '</div>'");
        sb.AppendLine("    ].join('');");
        sb.AppendLine("    document.body.appendChild(banner);");
        sb.AppendLine();
    ...
    ///     生成离线回退页面 HTML
    /// </summary>
    public static string generate_offline_page(string appName)
    {
        var sb = new StringBuilder();
        ...
        sb.AppendLine(
            "    p { color: #888; font-size: 14px; margin-bottom: 32px; max-width: 320px; line-height: 1.6; }");
        sb.AppendLine("    .retry {");
