...
public sealed class VoaErrorOverlay
{
    ...
    ///     渲染错误页面 HTML
    /// </summary>
    public string render_error_page(string error, string? filePath, int statusCode = 500)
    {
        ...
        sb.AppendLine("  <style>");
        sb.AppendLine(get_error_styles());
        sb.AppendLine("  </style>");
        sb.AppendLine("</head>");
        sb.AppendLine("<body>");
        ...
        {
            sb.AppendLine($"      <div class=\"voa-error-file\">{escape_html(filePath)}</div>");
        }

        sb.AppendLine($"      <pre class=\"voa-error-message\">{escape_html(error)}</pre>");
        sb.AppendLine("    </div>");
        sb.AppendLine("  </div>");
        sb.AppendLine("</body>");
        sb.AppendLine("</html>");
    ...
    /// </summary>
    public string render_structured_error_page(VoaErrorInfo errorInfo)
    {
        var sb = new StringBuilder();
        ...
        sb.AppendLine("  <style>");
        sb.AppendLine(get_error_styles());
        sb.AppendLine(get_structured_error_styles());
        sb.AppendLine("  </style>");
        sb.AppendLine("</head>");
        sb.AppendLine("<body>");
        sb.AppendLine("  <div class=\"voa-error-overlay\">");
        sb.AppendLine("    <div class=\"voa-error-header\">");
        sb.AppendLine(
            $"      <span class=\"voa-error-badge voa-error-badge-{errorInfo.severity}\">{errorInfo.severity_label}</span>");
        sb.AppendLine($"      <span class=\"voa-error-title\">{escape_html(errorInfo.title)}</span>");
        sb.AppendLine("    </div>");

        if (!string.IsNullOrEmpty(errorInfo.file_path))
        {
            sb.AppendLine("    <div class=\"voa-error-location\">");
            sb.AppendLine($"      <span class=\"voa-error-file\">{escape_html(errorInfo.file_path)}</span>");
            if (errorInfo.line > 0)
            {
                sb.AppendLine(
                    $"      <span class=\"voa-error-line\">行 {errorInfo.line}{(errorInfo.column > 0 ? $", 列 {errorInfo.column}" : "")}</span>");
            }
        ...
        sb.AppendLine("    <div class=\"voa-error-body\">");
        sb.AppendLine($"      <pre class=\"voa-error-message\">{escape_html(errorInfo.message)}</pre>");

        if (errorInfo.source_context is not null)
        {
            ...
            var startLine = Math.Max(1, errorInfo.source_context.start_line);
            var endLine = errorInfo.source_context.end_line;
            var lines = errorInfo.source_context.lines;
            ...
            for (var i = 0; i < lines.Length && startLine + i <= endLine; i++)
            {
                ...
                sb.AppendLine(
                    $"          <span class=\"voa-source-line {(isTarget ? "voa-source-line-error" : "")}\"><span class=\"voa-source-num\">{lineNum,4}</span> {escape_html(lines[i])}</span>");
            }
        ...
        if (errorInfo.related_errors is { Count: > 0 })
        {
            sb.AppendLine("      <div class=\"voa-error-related\">");
            sb.AppendLine("        <div class=\"voa-related-header\">相关错误</div>");

            foreach (var related in errorInfo.related_errors)
            {
                sb.AppendLine("        <div class=\"voa-related-item\">");
                sb.AppendLine(
                    $"          <span class=\"voa-related-badge voa-error-badge-{related.severity}\">{related.severity_label}</span>");
                if (!string.IsNullOrEmpty(related.file_path))
                {
                    sb.AppendLine(
                        $"          <span class=\"voa-related-file\">{escape_html(related.file_path)}</span>");
                }

                sb.AppendLine($"          <span class=\"voa-related-msg\">{escape_html(related.message)}</span>");
                sb.AppendLine("        </div>");
            }
        ...
        if (errorInfo.suggestions is { Count: > 0 })
        {
            ...
            {
                sb.AppendLine($"          <li>{escape_html(suggestion)}</li>");
            }
        ...
        if (!string.IsNullOrEmpty(errorInfo.documentation_url))
        {
            sb.AppendLine("      <div class=\"voa-error-docs\">");
            sb.AppendLine(
                $"        <a class=\"voa-docs-link\" href=\"{escape_html(errorInfo.documentation_url)}\" target=\"_blank\" rel=\"noopener\">📖 查看文档</a>");
            sb.AppendLine("      </div>");
        }

        if (errorInfo.stack_frames is { Count: > 0 })
        {
            ...
            foreach (var frame in errorInfo.stack_frames)
            {
                var frameClass = frame.is_native ? "voa-stack-native" : "voa-stack-frame";
                sb.AppendLine($"          <div class=\"{frameClass}\">");
                if (!string.IsNullOrEmpty(frame.file_path))
                {
                    sb.AppendLine($"            <span class=\"voa-stack-file\">{escape_html(frame.file_path)}</span>");
                    if (frame.line > 0)
                    {
                        sb.AppendLine(
                            $"            <span class=\"voa-stack-location\">:{frame.line}{(frame.column > 0 ? $":{frame.column}" : "")}</span>");
                    }
                }

                sb.AppendLine($"            <span class=\"voa-stack-fn\">{escape_html(frame.function_name)}</span>");
                sb.AppendLine("          </div>");
            }
    ...
    /// </summary>
    public string inject_hmr_script(string html, string host, int port)
    {
        var hmrScript = generate_hmr_client_script(host, port);
    ...
    /// </summary>
    private static string generate_hmr_client_script(string host, int port)
    {
        var sb = new StringBuilder();
        ...
        sb.AppendLine();
        sb.AppendLine(
            "    var containers = document.querySelectorAll('[data-component=\\\"' + componentName + '\\\"]');");
        ...
        sb.AppendLine();
        sb.AppendLine(
            "    var islandContainer = document.querySelector('[data-island][data-component=\\\"' + componentName + '\\\"]');");
        ...
        sb.AppendLine(
            "    html += '<span class=\"voa-overlay-badge\" style=\"background:' + sev.bg + '\">' + sev.label + '</span>';");
        ...
        sb.AppendLine(
            "        html += '<span class=\"voa-overlay-line\">行 ' + info.line + (info.column > 0 ? ', 列 ' + info.column : '') + '</span>';");
        sb.AppendLine("      }");
        sb.AppendLine("      html += '</div>';");
        sb.AppendLine("    }");
        sb.AppendLine();
        ...
        sb.AppendLine(
            "        html += '<div class=\"voa-source-line' + (isTarget ? ' voa-source-error' : '') + '\">';");
        sb.AppendLine("        html += '<span class=\"voa-source-num\">' + lineNum + '</span>';");
        sb.AppendLine(
            "        html += '<span class=\"voa-source-text\">' + escapeHtml(info.sourceLines[i]) + '</span>';");
        sb.AppendLine("        html += '</div>';");
        sb.AppendLine("      }");
        ...
        sb.AppendLine(
            "      html += '<div class=\"voa-related-label\">相关错误（' + info.relatedErrors.length + '）</div>';");
        ...
        sb.AppendLine(
            "        html += '<span class=\"voa-related-file\">' + escapeHtml(rel.filePath || '') + '</span>';");
        sb.AppendLine(
            "        html += '<span class=\"voa-related-msg\">' + escapeHtml(rel.message || '') + '</span>';");
        sb.AppendLine("        html += '</div>';");
        sb.AppendLine("      }");
        sb.AppendLine("      html += '</div>';");
        sb.AppendLine("    }");
    ...
    private static string get_error_styles()
    {
        var sb = new StringBuilder();
        ...
        sb.AppendLine(
            ".voa-error-code { background: #ff6b6b; color: #fff; padding: 4px 12px; border-radius: 4px; font-size: 18px; font-weight: bold; }");
        sb.AppendLine(".voa-error-title { font-size: 20px; color: #ffd93d; }");
        sb.AppendLine(
            ".voa-error-file { color: #4ecdc4; font-size: 14px; margin-bottom: 16px; padding: 8px 12px; background: #16213e; border-radius: 4px; }");
        sb.AppendLine(
            ".voa-error-message { background: #16213e; padding: 20px; border-radius: 8px; font-size: 14px; line-height: 1.6; overflow-x: auto; white-space: pre-wrap; border-left: 4px solid #ff6b6b; }");
        return sb.ToString();
    }

    private static string get_structured_error_styles()
    {
        var sb = new StringBuilder();
        sb.AppendLine(
            ".voa-error-badge { padding: 3px 10px; border-radius: 4px; font-size: 12px; font-weight: 600; color: #fff; }");
        ...
        sb.AppendLine(
            ".voa-error-location { display: flex; align-items: center; gap: 12px; margin-bottom: 16px; padding: 8px 12px; background: #16213e; border-radius: 4px; }");
        sb.AppendLine(".voa-error-line { color: #8b949e; font-size: 13px; }");
        sb.AppendLine(".voa-error-source { margin-top: 16px; }");
        sb.AppendLine(
            ".voa-source-header { font-size: 12px; color: #8b949e; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; }");
        sb.AppendLine(
            ".voa-source-code { background: #0d1117; padding: 12px; border-radius: 6px; overflow-x: auto; font-size: 13px; line-height: 1.6; }");
        sb.AppendLine(".voa-source-line { display: flex; gap: 12px; }");
        sb.AppendLine(
            ".voa-source-line-error { background: rgba(248, 81, 73, 0.15); border-left: 3px solid #ff6b6b; margin: 0 -12px; padding: 0 12px; }");
        ...
        sb.AppendLine(
            ".voa-related-header { font-size: 12px; color: #8b949e; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; }");
        sb.AppendLine(
            ".voa-related-item { display: flex; align-items: center; gap: 8px; padding: 6px 12px; border-radius: 4px; margin-bottom: 4px; background: #16213e; }");
        ...
        sb.AppendLine(
            ".voa-suggestions-header { font-size: 12px; color: #ffd93d; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; }");
        sb.AppendLine(".voa-suggestions-list { list-style: none; padding: 0; }");
        sb.AppendLine(
            ".voa-suggestions-list li { padding: 6px 12px; background: rgba(255, 217, 61, 0.08); border-left: 3px solid #ffd93d; margin-bottom: 4px; border-radius: 0 4px 4px 0; font-size: 13px; color: #e0e0e0; }");
        sb.AppendLine(".voa-error-docs { margin-top: 12px; }");
        sb.AppendLine(
            ".voa-docs-link { display: inline-flex; align-items: center; gap: 6px; padding: 6px 14px; background: rgba(88, 166, 255, 0.1); border: 1px solid rgba(88, 166, 255, 0.3); border-radius: 6px; color: #58a6ff; font-size: 13px; text-decoration: none; transition: all 0.2s; }");
        ...
        sb.AppendLine(
            ".voa-stack-header { font-size: 12px; color: #8b949e; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; }");
        sb.AppendLine(".voa-stack-frames { background: #0d1117; border-radius: 6px; overflow: hidden; }");
        sb.AppendLine(
            ".voa-stack-frame { display: flex; align-items: center; gap: 12px; padding: 6px 12px; border-bottom: 1px solid #21262d; font-size: 12px; }");
        sb.AppendLine(".voa-stack-frame:last-child { border-bottom: none; }");
        sb.AppendLine(
            ".voa-stack-native { display: flex; align-items: center; gap: 12px; padding: 6px 12px; background: rgba(110, 118, 129, 0.1); border-bottom: 1px solid #21262d; font-size: 12px; color: #6e7681; font-style: italic; }");
