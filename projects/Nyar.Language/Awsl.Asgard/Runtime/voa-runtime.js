(function (global) {
    ...
    class HydrationScheduler {
        ...
        schedule(config, element) {
            var id = element.dataset.islandId || 'island-' + Date.now() + '-' + Math.random().toString(36).slice(2);
        ...
        scheduleLazy(name, meta, strategy, element) {
            var id = element.dataset.islandId || 'island-' + Date.now() + '-' + Math.random().toString(36).slice(2);
            element.dataset.islandId = id;
            ...
            this.pending.set(id, lazyConfig);

            switch (strategy) {
                case 'idle':
                    this.idleCallback(function () {
                        this.hydrateLazy(lazyConfig);
                    }.bind(this));
                    break;
                case 'visible':
                    if (this.observer) {
                    ...
                    break;
                case 'interaction':
                    this.attachLazyInteractionListener(lazyConfig);
                    break;
                case 'media':
                    this.watchMediaQueryLazy(lazyConfig);
                    break;
                case 'load':
        ...
        attachLazyInteractionListener(lazyConfig) {
            var events = ['click', 'focus', 'input'];
            var handler = function () {
                events.forEach(function (e) {
                    lazyConfig.element.removeEventListener(e, handler);
                });
                this.hydrateLazy(lazyConfig);
            }.bind(this);
            events.forEach(function (e) {
                lazyConfig.element.addEventListener(e, handler, {once: true, passive: true});
            });
        }

        watchMediaQueryLazy(lazyConfig) {
            if (!lazyConfig.meta.mediaQuery) {
                this.hydrateLazy(lazyConfig);
                return;
            }

            var mq = matchMedia(lazyConfig.meta.mediaQuery);
            if (mq.matches) {
                this.hydrateLazy(lazyConfig);
            } else {
        ...
        hydrateLazy(lazyConfig) {
            if (lazyConfig.hydrated) return;
            lazyConfig.hydrated = true;
            this.pending.delete(lazyConfig.id);

            var self = this;
            loadComponent(lazyConfig.name).then(function () {
                var config = islandRegistry.get(lazyConfig.name);
                if (config) {
                    var island = {
                        id: lazyConfig.id,
                        config: config,
                        element: lazyConfig.element,
                        hydrated: true
                    };

                    self.performHydrate(island);
                }
            }).catch(function (e) {
                console.error('组件加载失败 ' + lazyConfig.name + ':', e);
            });
        }
        ...
        onVisible(entries) {
            entries.forEach(function (entry) {
                if (entry.isIntersecting) {
                    var pending = this.pending.get(entry.target.dataset.islandId);
                    if (pending && !pending.hydrated) {
                        this.observer.unobserve(entry.target);
                        if (pending.meta) {
                            this.hydrateLazy(pending);
                        } else {
                            this.hydrate(pending);
                        }
                    }
                }
            }.bind(this));
        ...
        performHydrate(island) {
            if (island.config.factory) {
                var rendered = island.config.factory(island.config.props);
                if (rendered) {
                    if (island.config.shadowDOM) {
                        var shadow = island.element.attachShadow({mode: 'open'});
            ...
            island.element.setAttribute('data-voa-hydrated', 'true');
        }
    ...
    function hydrateIslands() {
        if (_manifest) {
            hydrateIslandsFromManifest();
            return;
        }

        var manifestUrl = '/manifest.json';
        loadManifest(manifestUrl).then(function () {
            hydrateIslandsFromManifest();
        }).catch(function (e) {
            console.error('Manifest 加载失败，回退到 DOM 直接水合:', e);
            hydrateIslandsLegacy();
        });
    }

    function hydrateIslandsFromManifest() {
        ...
        for (var i = 0; i < elements.length; i++) {
            ...
            var meta = getComponentMeta(name);
            if (!meta) {
                continue;
            }

            var strategy = el.dataset.hydrate || meta.strategy || 'load';
            var scheduler = getScheduler();
            scheduler.scheduleLazy(name, meta, strategy, el);
        }
    }

    function hydrateIslandsLegacy() {
        var scheduler = getScheduler();
        var elements = document.querySelectorAll('[data-island]');

        elements.forEach(el => {
            var name = el.dataset.island;
            var strategy = el.dataset.hydrate || IslandStrategy.Load;
            var mediaQuery = el.dataset.media || null;

            var config = islandRegistry.get(name);
            if (config) {
                scheduler.schedule({
                    ...config,
                    strategy,
    ...
    function getComponentMeta(name) {
        ...
        for (var i = 0; i < _manifest.components.length; i++) {
            if (_manifest.components[i].name === name) {
                return _manifest.components[i];
            }
        }
        return null;
    }
    ...
    function registerComponent(name, meta) {
        _componentRegistry[name] = meta || {};
    }

    function loadComponent(name) {
        if (_componentLoaded[name]) {
            return _componentLoaded[name];
        }

        if (_componentLoading[name]) {
            return _componentLoading[name];
        }

        var meta = getComponentMeta(name);
        if (!meta) {
            var err = new Error('组件未在 manifest 中找到: ' + name);
            _componentLoading[name] = Promise.reject(err);
            return _componentLoading[name];
        }

        _componentLoading[name] = Promise.resolve().then(function () {
            var tasks = [];

            // 加载组件 CSS（dev 模式 per-component CSS）
            if (meta.css && !_cssLoaded[meta.css]) {
                tasks.push(loadComponentCss(meta.css));
            }

            // 加载组件 JS
            tasks.push(loadComponentJs(meta));

            return Promise.all(tasks).then(function () {
                return name;
            });
        }).then(function (name) {
            _componentLoaded[name] = _componentLoading[name];
            delete _componentLoading[name];
            return name;
        }).catch(function (e) {
            delete _componentLoading[name];
            throw e;
    ...
    function loadComponentJs(meta) {
        return new Promise(function (resolve, reject) {
            ...
            script.onerror = function () {
                var err = new Error('组件 JS 加载失败: ' + meta.js);
                reject(err);
            };
    ...
    function loadWasmFromManifest(name) {
        if (!_manifest || !_manifest.wasm) {
            return Promise.resolve(null);
        }

        var wasmInfo = null;
        for (var i = 0; i < _manifest.wasm.length; i++) {
            if (_manifest.wasm[i].name === name || !wasmInfo) {
                wasmInfo = _manifest.wasm[i];
            }
        }
        ...
        registerModule(name || wasmInfo.name, {
            wasmUrl: wasmInfo.url,
            glueUrl: wasmInfo.glue || null
        });

        return loadModule(name || wasmInfo.name);
    }
    ...
    function makeConsoleLog(memory) {
        return function (ptr) {
            ...
            var str = new TextDecoder('utf-8').decode(bytes.subarray(ptr, end));
            console.log(str);
        };
    }
    ...
    async function loadModule(name) {
        ...
        try {
            ...
            var importObject = Object.assign({},
                config.imports,
                {env: Object.assign({memory: memory, console_log: makeConsoleLog(memory)}, glue.imports || {})}
            );
    ...
    async function loadWasm(wasmUrl, imports) {
        ...
        var importObject = imports || {
            env: {
                memory: memory,
                console_log: makeConsoleLog(memory)
            }
        };
    ...
    function boot(config) {
        ...
        if (cfg.manifestUrl) {
            return loadManifest(cfg.manifestUrl).then(function (manifest) {
                if (cfg.autoLoadWasm && manifest.wasm && manifest.wasm.length > 0) {
                    var wasmName = manifest.module || manifest.wasm[0].name;
                    return loadWasmFromManifest(wasmName).then(function () {
                        return manifest;
                    });
                }
                return manifest;
        ...
        if (cfg.wasmUrl) {
            registerModule(cfg.moduleName || 'app', {
                wasmUrl: cfg.wasmUrl,
        ...
        if (cfg.appUrl) {
            ...
            script.onload = function () {
                if (cfg.autoInit !== false) {
                    loadModule(cfg.moduleName || 'app').catch(function (e) {
                        console.error('模块启动失败:', e);
                    });
                }
            };
            document.head.appendChild(script);
