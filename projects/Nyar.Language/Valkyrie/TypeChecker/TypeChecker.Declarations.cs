...
{
    #region 声明检查
    private void check_import_decl(ImportDecl import)
    {
    ...
    private void check_using_decl(UsingDecl usingDecl)
    {
    ...
    private void check_variable_decl(VariableDecl varDecl)
    {
    ...
    private void check_component_decl(ComponentDecl compDecl)
    {
        ...
        foreach (var field in compDecl.fields)
        {
            ...
            if (!fields.TryAdd(field.Name, fieldType))
            {
    ...
    private void check_system_decl(SystemDecl sysDecl)
    {
        ...
        {
            check_function_decl_in_scope(method, childScope);
        }
    ...
    private void check_widget_decl(WidgetDecl widgetDecl)
    {
        ...
        {
            var childScope = new Scope(_currentScope);
            check_function_decl_in_scope(widgetDecl.RenderMethod, childScope);
        }
    ...
    private void check_plugin_decl(PluginDecl pluginDecl)
    {
        var childScope = new Scope(_currentScope);

        foreach (var func in pluginDecl.Functions)
        {
            check_function_decl_in_scope(func, childScope);
        }
    ...
    private void check_function_decl(FunctionDecl funcDecl)
    {
        check_function_decl_in_scope(funcDecl, _currentScope);
    }

    private void check_function_decl_in_scope(FunctionDecl funcDecl, Scope parentScope)
    {
        ...
        foreach (var tp in funcDecl.TypeParameters)
        {
            ...
            var typeParameter = tp.Items[0];
            var constraintNames = funcDecl.GenericConstraints
                .Where(c => c.ParameterName == typeParameter.Name?.Name)
                .SelectMany(c => c.ConstraintTypes)
                .Select(c => c.Name)
                .ToList();

            var typeParameterName = typeParameter.Name?.Name ?? string.Empty;
            var typeVar = ValkyrieType.TypeVariable(typeParameterName, constraintNames);
            var typeSymbol = new Symbol(typeParameterName, typeVar, SymbolKind.Variable);
            funcScope.Define(typeSymbol);
        }

        if (funcDecl.GenericConstraints.Count > 0)
        {
            check_type_parameter_constraints_enhanced(funcDecl.TypeParameters, funcDecl.GenericConstraints);
        }

        foreach (var param in funcDecl.Parameters)
        {
            foreach (var paramItem in param.Items)
            {
                var paramName = paramItem.Name?.Name ?? string.Empty;
                var paramType = ResolveTypeAnnotation(paramItem.BoundType);
                paramTypes.Add(new ParameterType(paramName, paramType));

                var paramSymbol = new Symbol(paramName, paramType, SymbolKind.Parameter);
                if (!funcScope.Define(paramSymbol))
                {
                    AddError("VALK2010", $"参数 '{paramName}' 在函数 '{funcDecl.Name}' 中重复定义", paramItem.Span, "重命名参数以避免重复定义");
                }
            }
        }
        ...
        var genericTypeParamNames = funcDecl.TypeParameters
            .SelectMany(t => t.Items)
            .Select(t => t.Name?.Name ?? string.Empty)
            .ToList();
    ...
    /// </summary>
    private void check_trait_decl(TraitDecl traitDecl)
    {
        ...
        {
            check_type_parameter_constraints_enhanced(traitDecl.TypeParameters, traitDecl.GenericConstraints);
        }
        ...
        foreach (var method in traitDecl.Methods)
        {
            ...
            foreach (var param in method.Parameters)
            {
                ...
                {
                    var paramType = ResolveTypeAnnotation(paramItem.BoundType);
                    paramTypes.Add(new ParameterType(paramItem.Name?.Name ?? string.Empty, paramType));
                }
            }
        ...
        register_trait_decl(traitDecl.Name, traitType);
    }

    /// <summary>
    /// 检查枚举声明：验证成员值类型一致性，注册枚举类型和成员符号    /// </summary>
    private void check_enum_decl(EnumDecl enumDecl)
    {
        ...
        foreach (var member in enumDecl.Members)
        {
            ...
            if (member.Value is not null)
            {
                ...
                if (!valueType.IsError && !valueType.IsInteger && !valueType.IsTextLike)
                {
                    AddError("VALK2061",
                        $"枚举 '{enumDecl.Name}' 的成员 '{member.Name}' 值类型不合法：期望整数或文本，实际为 '{valueType}'",
                        member.Span, "枚举成员值必须是整数或文本字面量");
                }
            }
    ...
    /// </summary>
    private void check_flags_decl(FlagsDecl flagsDecl)
    {
        var variantNames = new HashSet<string>(StringComparer.Ordinal);
        var variantTypes = new List<ValkyrieType>();
    ...
    /// </summary>
    private void check_union_decl(UnionDecl unionDecl)
    {
        var variantNames = new HashSet<string>(StringComparer.Ordinal);
        var variantTypes = new List<ValkyrieType>();
    ...
    /// </summary>
    private void check_unite_decl(UniteDecl uniteDecl)
    {
        var variantNames = new HashSet<string>(StringComparer.Ordinal);
        var variantTypes = new List<ValkyrieType>();
    ...
    private void check_type_alias_decl(TypeAliasDecl typeAliasDecl)
    {
    ...
    /// </summary>
    private void check_namespace_decl(NamespaceDecl namespaceDecl)
    {
    ...
    #region 着色器声明检查
    private void check_shader_decl(ShaderDecl shaderDecl)
    {
        var shaderScope = new Scope(_currentScope);

        foreach (var stage in shaderDecl.Stages)
        {
            check_shader_stage_decl(stage, shaderScope);
        }
    ...
    private void check_shader_stage_decl(ShaderStageDecl stage, Scope shaderScope)
    {
        ...
        foreach (var bodyNode in stage.Body)
        {
            switch (bodyNode)
            {
                case UniformDecl uniform:
                    check_uniform_decl_in_scope(uniform, stageScope);
                    break;
                case VaryingDecl varying:
                    check_varying_decl_in_scope(varying, stageScope);
                    break;
                case ConstantBufferDecl cbuffer:
                    check_constant_buffer_decl_in_scope(cbuffer, stageScope);
                    break;
                case TextureDecl texture:
                    check_texture_decl_in_scope(texture, stageScope);
                    break;
                case SamplerDecl sampler:
                    check_sampler_decl_in_scope(sampler, stageScope);
                    break;
                case ShaderAttributeDecl attr:
                    check_shader_attribute_decl_in_scope(attr, stageScope);
                    break;
                case FunctionDecl func:
                    check_function_decl_in_scope(func, stageScope);
                    break;
                case VariableDecl varDecl:
                    check_variable_decl(varDecl);
                    break;
                case StructureDecl structDecl:
                    check_struct_decl(structDecl);
                    break;
                default:
    ...
    private void check_struct_decl(StructureDecl structDecl)
    {
        var fields = new Dictionary<string, ValkyrieType>(StringComparer.Ordinal);
    ...
    private void check_class_decl(ClassDecl classDecl)
    {
        ...
        foreach (var func in classDecl.Functions)
        {
            ...
            foreach (var param in func.Parameters)
            {
                foreach (var paramItem in param.Items)
                {
                    ResolveTypeAnnotation(paramItem.BoundType);
                }
            }
        }
    ...
    private void check_uniform_decl(UniformDecl uniformDecl)
    {
        check_uniform_decl_in_scope(uniformDecl, _currentScope);
    }

    private void check_uniform_decl_in_scope(UniformDecl uniformDecl, Scope scope)
    {
    ...
    private void check_varying_decl(VaryingDecl varyingDecl)
    {
        check_varying_decl_in_scope(varyingDecl, _currentScope);
    }

    private void check_varying_decl_in_scope(VaryingDecl varyingDecl, Scope scope)
    {
    ...
    private void check_constant_buffer_decl(ConstantBufferDecl cbufferDecl)
    {
        check_constant_buffer_decl_in_scope(cbufferDecl, _currentScope);
    }

    private void check_constant_buffer_decl_in_scope(ConstantBufferDecl cbufferDecl, Scope scope)
    {
        var fields = new Dictionary<string, ValkyrieType>(StringComparer.Ordinal);
    ...
    private void check_texture_decl(TextureDecl textureDecl)
    {
        check_texture_decl_in_scope(textureDecl, _currentScope);
    }

    private void check_texture_decl_in_scope(TextureDecl textureDecl, Scope scope)
    {
    ...
    private void check_sampler_decl(SamplerDecl samplerDecl)
    {
        check_sampler_decl_in_scope(samplerDecl, _currentScope);
    }

    private void check_sampler_decl_in_scope(SamplerDecl samplerDecl, Scope scope)
    {
    ...
    private void check_uniform_binding_decl(UniformBindingDecl uniformBindingDecl)
    {
    ...
    private void check_shader_attribute_decl(ShaderAttributeDecl shaderAttrDecl)
    {
        check_shader_attribute_decl_in_scope(shaderAttrDecl, _currentScope);
    }

    private void check_shader_attribute_decl_in_scope(ShaderAttributeDecl shaderAttrDecl, Scope scope)
    {
    ...
    private ValkyrieType infer_swizzle_type(SwizzleExpr swizzle)
    {
        ...
        if (!is_valid_swizzle(targetType, swizzle.Components))
        {
            AddError("VALK2041",
        ...
        {
            1 => get_swizzle_scalar_type(targetType),
            2 => get_swizzle_vec_type(targetType, 2),
            3 => get_swizzle_vec_type(targetType, 3),
            4 => get_swizzle_vec_type(targetType, 4),
            _ => ValkyrieType.Error
        };
    }

    private static bool is_valid_swizzle(ValkyrieType targetType, string components)
    {
    ...
    private static ValkyrieType get_swizzle_scalar_type(ValkyrieType targetType)
    {
    ...
    private static ValkyrieType get_swizzle_vec_type(ValkyrieType targetType, int componentCount)
    {
