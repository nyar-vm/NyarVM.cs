...
public sealed partial class TypeChecker
{
    ...
    /// </summary>
    private List<GenericConstraintDescriptor> resolve_constraints(
    ...
    /// </summary>
    private void check_generic_constraints_enhanced(
        IReadOnlyList<TypeParameter> typeParameters,
        IReadOnlyList<GenericConstraint> genericConstraints,
        TermCallExpression call)
    {
        var descriptors = resolve_constraints(genericConstraints, typeParameters);
        ...
        foreach (var tp in typeParameters)
        {
            ...
            var paramConstraints = get_constraints_for_parameter(tp.Name, genericConstraints, descriptors);
    ...
    /// 获取指定类型参数的所有约束描述符
    /// </summary>
    private List<GenericConstraintDescriptor> get_constraints_for_parameter(
        string paramName,
        IReadOnlyList<GenericConstraint> genericConstraints,
    ...
    /// </summary>
    private void check_type_parameter_constraints_enhanced(
        IReadOnlyList<TypeParameter> typeParameters,
        IReadOnlyList<GenericConstraint> genericConstraints)
    {
        foreach (var constraint in genericConstraints)
        {
            ...
            {
                if (is_builtin_constraint_kind(ct.Name))
                {
                    continue;
                }
        ...
        check_constraint_combination_validity(typeParameters, genericConstraints);
    }
    ...
    /// </summary>
    private void check_constraint_combination_validity(
        IReadOnlyList<TypeParameter> typeParameters,
        IReadOnlyList<GenericConstraint> genericConstraints)
    {
        foreach (var tp in typeParameters)
    ...
    /// 判断约束名是否为内置约束种类
    /// </summary>
    private static bool is_builtin_constraint_kind(string name)
    {
    ...
    /// </summary>
    private void check_generic_constraints_from_func_type(ValkyrieType funcType, TermCallExpression call)
    {
    ...
    /// 注册 trait 声明到类型注册表
    /// </summary>
    private void register_trait_decl(string name, ValkyrieType traitType)
    {
    ...
    /// </summary>
    private void register_trait_implementation(string typeName, string traitName)
    {
    ...
    /// 用于泛型约束求解和类型测试
    /// </summary>
    private bool verify_trait_implementation(ValkyrieType type, string traitName)
    {
