...
{
    private ValkyrieType infer_index_type(TermIndexExpression index)
    {
        return infer_offset_index_type(new OffsetIndexExpression
        {
            Target = index.Target,
    ...
    private ValkyrieType infer_ordinal_index_type(OrdinalIndexExpression index)
    {
        var targetType = InferType(index.Target);
        var ordinalTypes = index.Indices.Select(InferType).ToArray();

        if (targetType.Kind == TypeKind.Array || targetType.Kind == TypeKind.FixedArray || targetType.Name == "Array" || targetType.Name == "FixedArray")
        {
    ...
    private ValkyrieType infer_offset_index_type(OffsetIndexExpression index)
    {
        var targetType = InferType(index.Target);
