...
/// </remarks>
[Obsolete(
    "Ed25519 自建实现不符合 RFC 8032 标准，请迁移到 NSec.Cryptography 库。Verification 端（Valhalla.Server/Ed25519AuthMiddleware.cs）已使用 NSec 正确实现。")]
public class SignatureResult
{
    public bool success { get; set; }
    public byte[] signature_bytes { get; set; } = [];
    public string? error { get; set; }

    public static SignatureResult ok(byte[] signature)
    {
        return new SignatureResult { success = true, signature_bytes = signature };
    }

    public static SignatureResult err(string error)
    {
        return new SignatureResult { success = false, error = error };
    }
}