...
public sealed class WasiIntegrationTests
{
    ...
    public void WasiOverall_Coverage_AllCategoriesPresent()
    {
        ...
        foreach (var (category, count) in categories) _output.WriteLine($"  - {category}：{count} 个函数");
    ...
    private static int wasi_random_bytes(byte[] buffer, int length)
    {
        return 0;
    }

    private static uint wasi_random_u32()
    {
        return 42;
    }

    private static int wasi_exit(int code)
    {
        return code;
    }

    private static int wasi_sleep_ms(long ms)
    {
        return 0;
    }
    ...
    public void FileSystem_PathOperations_Exist()
    {
        ...
        foreach (var op in operations) Assert.NotNull(op);
    ...
    {
        var apis = new[]
            { "wasi_fs_read_text", "wasi_fs_write_text", "wasi_fs_exists", "wasi_fs_delete", "wasi_fs_list_dir" };
