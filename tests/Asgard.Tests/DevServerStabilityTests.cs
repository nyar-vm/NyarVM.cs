...
public sealed class DevServerStabilityTests
{
    ...
    public async Task SimulatedRuntime_OneHour_NoMemoryLeak()
    {
        ...
        _output.WriteLine(
            $"内存：初始 {initialMemory / 1024 / 1024}MB → 最终 {finalMemory / 1024 / 1024}MB（增长 {growthMb:F1}MB）");
    ...
    public void Monitor_HealthReport_FormatCorrectly()
    {
        ...
        _output.WriteLine(
            $"健康报告：运行={report.UptimeFormatted} 请求={report.TotalRequests} 错误率={report.ErrorRate:P1} 健康={report.IsHealthy}");
    }
    ...
    public void ErrorOverlay_StackFrames_ReturnedWhenAvailable()
    {
        ...
        var info = new VoaErrorInfo
        {
            ...
            StatusCode = 500,
            Suggestions = ["检查变量 'x' 是否已声明", "确认导入路径正确"],
    ...
    public void HealthCheck_HealthyServer_ReturnsTrue()
    {
        ...
        for (var i = 0; i < 100; i++) monitor.RecordRequest(1024);
