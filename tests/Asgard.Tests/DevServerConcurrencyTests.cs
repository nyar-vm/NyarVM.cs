...
public sealed class DevServerConcurrencyTests
{
    ...
    public void ConcurrentRequests_Throughput_MeasuresRps()
    {
        ...
        for (var i = 0; i < requestCount; i++) monitor.RecordRequest(1024);
    ...
    public void WebSockets_Tracking_MultipleClients()
    {
        ...
        for (var i = 0; i < 50; i++) clients[$"client_{i}"] = true;
    ...
    {
        var clients = new ConcurrentDictionary<string, bool>();
        for (var i = 0; i < 20; i++) clients[$"client_{i}"] = true;

        var deadIds = new[] { "client_3", "client_7", "client_15", "client_19" };
        foreach (var id in deadIds) clients.TryRemove(id, out _);

        Assert.Equal(16, clients.Count);
        foreach (var id in deadIds) Assert.False(clients.ContainsKey(id), $"已断开客户端 {id} 应从列表中移除");
    ...
    public void LockContention_Minimize_WithConcurrentCollections()
    {
        ...
        Parallel.For(0, 1000, i => { dict[i] = i * 2L; });
    ...
    public async Task RequestQueue_BoundedCapacity_PreventsOverload()
    {
        ...
        for (var i = 0; i < 100; i++) await queue.Writer.WriteAsync(i);
    ...
    public async Task RequestQueue_FairScheduling_ProcessesAll()
    {
        ...
        for (var i = 0; i < 50; i++) await queue.Writer.WriteAsync(i);

        queue.Writer.Complete();

        var processed = 0;
        await foreach (var item in queue.Reader.ReadAllAsync()) processed++;
