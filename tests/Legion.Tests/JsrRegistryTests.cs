...
{
    private const string _jsr_package_json = @"{
            ""scope"": ""testscope"",
            ""name"": ""testpkg"",
            ""latestVersion"": ""1.2.3"",
            ""description"": ""jsr 测试包"",
    ...
    public async Task GetPackageAsync_LatestVersion_ReturnsCorrectPackage()
    {
        ...
        handler.add_route("https://jsr.io/api/packages/@testscope%2Ftestpkg",
            HttpStatusCode.OK, _jsr_package_json);
        handler.add_route("https://jsr.io/api/packages/@testscope%2Ftestpkg/versions/1.2.3",
            HttpStatusCode.OK, @"{
                    ""version"": ""1.2.3"",
                    ""manifest"": {
                        ""dependencies"": {
    ...
    public async Task GetPackageAsync_NotFound_ThrowsRegistryException()
    {
        ...
        await Assert.ThrowsAsync<RegistryException>(() => registry.GetPackageAsync("nonexistent", "latest"));
    }
    ...
    public async Task SearchPackagesAsync_ReturnsResults()
    {
        ...
        var handler = new RoutingFakeHttpMessageHandler();
        handler.add_route("https://jsr.io/api/packages?query=test", HttpStatusCode.OK, searchJson);
    ...
    public async Task GetPackageVersionsAsync_ReturnsVersionList()
    {
        ...
        var handler = new RoutingFakeHttpMessageHandler();
        handler.add_route("https://jsr.io/api/packages/@testscope%2Ftestpkg/versions?limit=100",
