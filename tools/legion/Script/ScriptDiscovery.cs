...
public class ScriptDiscovery
{
    ...
    public Dictionary<string, string> discover_scripts(string projectDir)
    {
        var result = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        var scriptDir = Path.Combine(projectDir, "script");

        if (!Directory.Exists(scriptDir)) return result;
    ...
    public bool has_script(string projectDir, string scriptName)
    {
        var scripts = discover_scripts(projectDir);
