...
public sealed class ModuleResolver
{
    ...
    public string? resolve_module(string moduleName, string currentFilePath, string projectRoot)
    {
        if (string.IsNullOrWhiteSpace(moduleName)) return null;

        var searchPaths = build_search_paths(currentFilePath, projectRoot);

        foreach (var searchPath in searchPaths)
        {
            var resolved = try_resolve_in_path(moduleName, searchPath);
    ...
    private static string? try_resolve_in_path(string moduleName, string searchPath)
    {
        var moduleDir = Path.Combine(searchPath, moduleName);
        if (Directory.Exists(moduleDir)) return moduleDir;

        var moduleFile = Path.Combine(searchPath, $"{moduleName}.v");
        if (File.Exists(moduleFile)) return moduleFile;
    ...
    private static List<string> build_search_paths(string currentFilePath, string projectRoot)
    {
        var paths = new List<string>();

        var currentDir = Path.GetDirectoryName(currentFilePath);
        if (!string.IsNullOrEmpty(currentDir)) paths.Add(currentDir);
        ...
        if (!string.IsNullOrEmpty(projectRoot))
        {
            ...
            if (Directory.Exists(projectVendors)) paths.Add(projectVendors);
        }

        var globalVendors = resolve_global_vendors_path();
        if (globalVendors is not null && !paths.Contains(globalVendors)) paths.Add(globalVendors);
    ...
    private static string? resolve_global_vendors_path()
    {
        ...
        if (!string.IsNullOrEmpty(valkyrieHome))
        {
            ...
            if (Directory.Exists(vendorsPath)) return vendorsPath;
        }
        ...
        if (!string.IsNullOrEmpty(homeDir))
        {
            ...
            if (Directory.Exists(defaultVendors)) return defaultVendors;
        }
